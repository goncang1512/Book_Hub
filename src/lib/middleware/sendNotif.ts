...
export const newChapterBook = async (book_id: string, senderId: string, canvas: any) => {
  ...
  for (const list of whislist) {
    let message = `
        ...
        <p><br></p>
        <p>Klik di sini untuk membaca: <a style="color: blue;" href="/read/${book_id}">${canvas?.judul}</a></p>
